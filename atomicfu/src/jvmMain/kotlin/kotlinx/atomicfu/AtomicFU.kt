 ...
 * ```
 */
public actual fun <T> atomic(initial: T, trace: Trace): AtomicRef<T> = AtomicRef<T>(initial, trace)
 ...
 * ```
 */
public actual fun atomic(initial: Int, trace: Trace): AtomicInt = AtomicInt(initial, trace)
 ...
 * ```
 */
public actual fun atomic(initial: Long, trace: Trace): AtomicLong = AtomicLong(initial, trace)
 ...
 * ```
 */
public actual fun atomic(initial: Boolean, trace: Trace): AtomicBoolean = AtomicBoolean(initial, trace)
...
public actual class AtomicRef<T> internal constructor(value: T, val trace: Trace) {
    /**
    ...
    public actual var value: T = value
        set(value) {
            ...
            field = value
            trace.append("value set to $value")
    ...
    public actual fun compareAndSet(expect: T, update: T): Boolean {
        ...
        if (result) {
            trace.append("value CAS from $expect to $update")
    ...
    public actual fun getAndSet(value: T): T {
        ...
        trace.append("value set from $oldValue to $value")
 ...
 */
@Suppress("UNCHECKED_CAST")
public actual class AtomicBoolean internal constructor(v: Boolean, val trace: Trace) {
 ...
 * like [compareAndSet] and others.
 */
public actual class AtomicInt internal constructor(value: Int, val trace: Trace) {
    /**
 ...
 * like [compareAndSet] and others.
 */
public actual class AtomicLong internal constructor(value: Long, val trace: Trace) {
    /**
     * Reads/writes of this property maps to read/write of volatile variable.
     */
    @Volatile
